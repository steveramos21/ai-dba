...
async function testEngine(engineId, engineConfig, connector) {
  ...
  try {
    ...
    assert(`${label} describeTable has id column`, cols.some(c => c.name === 'id'), `got: ${cols.map(c=>c.name).join(',')}`);
  ...
  try {
    ...
    assert(`${label} listIndexes PK has id column`, pk?.columns.includes('id'), `got: ${pk?.columns}`);
  ...
  try {
    ...
    // At least our own connection should show (or not, since we exclude ourselves)
    // Just verify structure if any results
    if (procs.length > 0) {
  ...
  // 7. query (read-only)
  try {
    const result = await connector.query(engineId, engineConfig, 'SELECT 1 as val');
    assert(`${label} query SELECT 1 returns columns`, result.columns.includes('val'));
    assert(`${label} query SELECT 1 returns rows`, result.rows.length === 1);
    assert(`${label} query SELECT 1 value is 1`, Number(result.rows[0].val) === 1);
